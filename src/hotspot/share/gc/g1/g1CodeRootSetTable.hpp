...
class G1CodeRootSetTable : public Hashtable<CompiledMethod*, mtGC> {
  friend class G1CodeRootSetTest;
  typedef HashtableEntry<CompiledMethod*, mtGC> Entry;

  static G1CodeRootSetTable* volatile _purge_list;

  G1CodeRootSetTable* _purge_next;

  unsigned int compute_hash(CompiledMethod* nm) {
  ...
  Entry* new_entry(CompiledMethod* nm);

 public:
  G1CodeRootSetTable(int size) : Hashtable<CompiledMethod*, mtGC>(size, sizeof(Entry)), _purge_next(NULL) {}
  ~G1CodeRootSetTable();

  // Needs to be protected by locks
  bool add(CompiledMethod* nm);
  bool remove(nmethod* nm);

  // Can be called without locking
  bool contains(CompiledMethod* nm);
