...
const Type* SubTypeCheckNode::sub_old(const Type* sub_t, const Type* super_t) const {
...
const Type* SubTypeCheckNode::sub(const Type* sub_t, const Type* super_t) const {
  ...
  if (subk != NULL) {
    switch (Compile::current()->static_subtype_check(superk, subk)) {
      case Compile::SSC_always_false:
        ...
        return TypeInt::CC_GT;
      case Compile::SSC_always_true:
        assert(old == TypeInt::CC_EQ || old == NULL, "");
        return TypeInt::CC_EQ;
      case Compile::SSC_easy_test:
      case Compile::SSC_full_test:
        break;
  ...
  assert(old == bottom_type() || old == NULL || StressReflectiveCode, "");
  return bottom_type();
}
