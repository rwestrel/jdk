...
bool LibraryCallKit::inline_Class_cast() {
  ...
  if (tm != NULL && tm->is_klass() &&
    ...
    if (!tp->is_loaded()) {
      ...
      int static_res = C->static_subtype_check(TypeKlassPtr::make(tm->as_klass()), tp->as_klass_type()->cast_to_exactness(false));
  ...
  { PreserveReexecuteState preexecs(this);
    ...
    if (!stopped()) {
      ...
      if (_gvn.type(klass_node)->singleton()) {
        ...
        int test = C->static_subtype_check(superk, subk->cast_to_exactness(false));
