...
const Type *CmpPNode::sub( const Type *t1, const Type *t2 ) const {
  const Type* res1 = sub_new(t1, t2);
  const Type* res2 = sub_old(t1, t2);
  assert(res1 == res2, "");
  return res1;
}
...
// If both inputs are constants, compare them.
const Type *CmpPNode::sub_new( const Type *t1, const Type *t2 ) const {
...
const Type *CmpPNode::sub_old( const Type *t1, const Type *t2 ) const {
  const TypePtr *r0 = t1->is_ptr(); // Handy access
  const TypePtr *r1 = t2->is_ptr();
