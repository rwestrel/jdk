...
bool CompilationPolicy::must_be_compiled(const methodHandle& m, jlong profile_context, int comp_level) {
  ...
  if (m->has_compiled_code(profile_context)) return false;       // already compiled
...
void CompilationPolicy::maybe_compile_early(const methodHandle& m, jlong profile_context, TRAPS) {
  ...
  if (!m->is_native() && MethodTrainingData::have_data()) {
    ...
    CompLevel next_level = trained_transition(m, cur_level, mtd, profile_context, THREAD);
    if (next_level != cur_level && can_be_compiled(m, next_level) && !CompileBroker::compilation_is_in_queue(m)) {
      if (PrintTieredEvents) {
        print_event(FORCE_COMPILE, m(), m(), InvocationEntryBci, next_level, profile_context);
      }
      CompileBroker::compile_method(m, InvocationEntryBci, next_level, 0, CompileTask::Reason_MustBeCompiled, profile_context, THREAD);
...
void CompilationPolicy::compile_if_required(const methodHandle& m, jlong profile_context, TRAPS) {
  ...
  if (must_be_compiled(m, profile_context)) {
    // This path is unusual, mostly used by the '-Xcomp' stress test mode.
    CompLevel level = initial_compile_level(m);
    if (PrintTieredEvents) {
      print_event(FORCE_COMPILE, m(), m(), InvocationEntryBci, level, profile_context);
    }
    CompileBroker::compile_method(m, InvocationEntryBci, level, 0, CompileTask::Reason_MustBeCompiled, profile_context, THREAD);
  }
}
        ...
        ktd->iterate_comp_deps([&](CompileTrainingData* ctd) {
          if (ctd->init_deps_left_acquire() == 0) {
            ...
            if (mtd->has_holder()) {
              ...
              CompilationPolicy::maybe_compile_early(mh, profile_context, current);
            }
          }
        });
      }
...
void CompilationPolicy::replay_training_at_init_loop(jlong profile_context, JavaThread* current) {
  while (!CompileBroker::is_compilation_disabled_forever()) {
    ...
    if (ik != nullptr) {
      replay_training_at_init_impl(ik, profile_context, current);
    }
  }
}
...
void CompilationPolicy::print_training_data_on(outputStream* st,  const char* prefix, Method* method, CompLevel cur_level, jlong profile_context) {
  ...
  if (mtd == nullptr) {
    ...
    if (should_delay_standard_transition(m, cur_level, mtd, profile_context)) {
      st->print("delayed, ");
    }
...
void CompilationPolicy::print_event_on(outputStream *st, EventType type, Method* m, Method* im, int bci, CompLevel level, jlong profile_context) {
  ...
  st->print(" rate=");
  if (m->prev_time(profile_context) == 0) st->print("n/a");
  else st->print("%f", m->rate(profile_context));

  st->print(" k=%.2lf,%.2lf", threshold_scale(CompLevel_full_profile, Tier3LoadFeedback),
                              threshold_scale(CompLevel_full_optimization, Tier4LoadFeedback));

  if (type != COMPILE) {
    print_counters_on(st, "", m, profile_context);
    if (inlinee_event) {
      print_counters_on(st, "inlinee ", im, profile_context);
    }
    st->print(" compilable=");
    ...
    print_training_data_on(st, "", m, level, profile_context);
    if (inlinee_event) {
      print_training_data_on(st, "inlinee ", im, level, profile_context);
    }
  }
  st->print_cr("]");
...
CompileTask* CompilationPolicy::select_task(CompileQueue* compile_queue, JavaThread* THREAD) {
  ...
  for (CompileTask* task = compile_queue->first(); task != nullptr;) {
    ...
    update_rate(t, mh, profile_context);
    if (max_task == nullptr || compare_methods(method, max_method, profile_context)) {
      // Select a method with the highest rate
      max_task = task;
      max_method = method;
      max_profile_context = profile_context;
    }

    if (task->is_blocking()) {
      if (max_blocking_task == nullptr || compare_methods(method, max_blocking_task->method(), profile_context)) {
  ...
  if (max_blocking_task != nullptr) {
    ...
    max_profile_context = max_task->profile_context();
  }
  ...
  if (max_task != nullptr && max_method != nullptr) {
    ...
    if (max_task->comp_level() == CompLevel_full_profile && TieredStopAtLevel > CompLevel_full_profile &&
        is_method_profiled(max_method_h, max_profile_context) && !Arguments::is_compiler_only()) {
      ...
      if (!exclude_limited_profile) {
        ...
        if (CompileBroker::compilation_is_complete(max_method_h, max_task->osr_bci(), CompLevel_limited_profile, max_profile_context)) {
          if (PrintTieredEvents) {
            print_event(REMOVE_FROM_QUEUE, max_method, max_method, max_task->osr_bci(), (CompLevel)max_task->comp_level(), max_profile_context);
          }
        ...
        if (PrintTieredEvents) {
          print_event(UPDATE_IN_QUEUE, max_method, max_method, max_task->osr_bci(), (CompLevel)max_task->comp_level(), max_profile_context);
        }
      }
    }
  }
...
void CompilationPolicy::reprofile(ScopeDesc* trap_scope, bool is_osr, jlong profile_context) {
  for (ScopeDesc* sd = trap_scope;; sd = sd->sender()) {
    ...
    MethodData* mdo = sd->method()->method_data(profile_context);
    if (mdo != nullptr) {
...
nmethod* CompilationPolicy::event(const methodHandle& method, const methodHandle& inlinee,
  ...
  handle_counter_overflow(method, profile_context);
  if (method() != inlinee()) {
    handle_counter_overflow(inlinee, profile_context);
  }

  if (bci == InvocationEntryBci) {
    method_invocation_event(method, inlinee, comp_level, nm, profile_context, THREAD);
  } else {
    // method == inlinee if the event originated in the main method
    method_back_branch_event(method, inlinee, bci, comp_level, nm, profile_context, THREAD);
...
void CompilationPolicy::compile(const methodHandle& mh, int bci, CompLevel level, jlong profile_context,
  ...
  if (level == CompLevel_none) {
    if (mh->has_compiled_code(profile_context)) {
      // Happens when we switch to interpreter to profile.
      MutexLocker ml(Compile_lock);
      NoSafepointVerifier nsv;
      if (mh->has_compiled_code(profile_context)) {
        mh->code(profile_context)->make_not_used();
      }
  ...
  if ((bci == InvocationEntryBci && !can_be_compiled(mh, level))) {
    if (!CompilationModeFlag::disable_intermediate() &&
      ...
      compile(mh, bci, CompLevel_simple, profile_context, THREAD);
    }
    return;
  }
  ...
  if (!CompileBroker::compilation_is_in_queue(mh)) {
    if (PrintTieredEvents) {
      print_event(COMPILE, mh(), mh(), bci, level, profile_context);
    }
    int hot_count = (bci == InvocationEntryBci) ? mh->invocation_count(profile_context) : mh->backedge_count(profile_context);
    update_rate(nanos_to_millis(os::javaTimeNanos()), mh, profile_context);
    CompileBroker::compile_method(mh, bci, level, hot_count, CompileTask::Reason_Tiered, profile_context, THREAD);
  }
}
...
void CompilationPolicy::update_rate(int64_t t, const methodHandle& method, jlong profile_context) {
  ...
  if (is_old(method, profile_context)) {
    // We don't remove old methods from the queue,
    // so we can just zero the rate.
    method->set_rate(0, profile_context);
    return;
  }
  ...
  int64_t delta_t = t - (method->prev_time(profile_context) != 0 ? method->prev_time(profile_context) : start_time()); // milliseconds since the last measurement
  // How many events were there since the last time?
  int event_count = method->invocation_count(profile_context) + method->backedge_count(profile_context);
  int delta_e = event_count - method->prev_event_count(profile_context);

  // We should be running for at least 1ms.
  if (delta_s >= TieredRateUpdateMinTime) {
    // And we must've taken the previous point at least 1ms before.
    if (delta_t >= TieredRateUpdateMinTime && delta_e > 0) {
      method->set_prev_time(t, profile_context);
      method->set_prev_event_count(event_count, profile_context);
      method->set_rate((float) delta_e / (float) delta_t, profile_context); // Rate is events per millisecond
    } else {
...
bool CompilationPolicy::compare_methods(Method* x, Method* y, jlong profile_context) {
  if (x->highest_comp_level() > y->highest_comp_level()) {
    ...
    if (x->highest_comp_level() == y->highest_comp_level()) {
      if (weight(x, profile_context) > weight(y, profile_context)) {
        return true;
      }
    }
  return false;
...
void CompilationPolicy::create_mdo(const methodHandle &mh, jlong profile_context, JavaThread* THREAD) {
  ...
  if (mh->method_data(profile_context) == nullptr) {
    Method::build_profiling_method_data(mh, profile_context, CHECK_AND_CLEAR);
  }
  if (ProfileInterpreter && THREAD->has_last_Java_frame()) {
    MethodData* mdo = mh->method_data(profile_context);
    if (mdo != nullptr) {
...
CompLevel CompilationPolicy::trained_transition_from_none(const methodHandle& method, CompLevel cur_level, MethodTrainingData* mtd, jlong profile_context, JavaThread* THREAD) {
  ...
  if (deopt && !is_method_profiled(method, profile_context)) {
    return CompLevel_full_profile;
  }
  ...
  if (SkipTier2IfPossible && ctd->init_deps_left_acquire() == 0) {
    if (method->method_data(profile_context) == nullptr) {
      create_mdo(method, profile_context, THREAD);
    }
...
CompLevel CompilationPolicy::trained_transition_from_full_profile(const methodHandle& method, CompLevel cur_level, MethodTrainingData* mtd, jlong profile_context, JavaThread* THREAD) {
  ...
  if (highest_training_level == CompLevel_full_optimization && !mtd->only_inlined()) {
    ...
    if (is_method_profiled(method, profile_context)) {
      return CompLevel_full_optimization;
    }
  }
...
CompLevel CompilationPolicy::trained_transition(const methodHandle& method, CompLevel cur_level, MethodTrainingData* mtd, jlong profile_context, JavaThread* THREAD) {
  ...
  switch(cur_level) {
    default: break;
    case CompLevel_none:
      next_level = trained_transition_from_none(method, cur_level, mtd, profile_context, THREAD);
      break;
    case CompLevel_limited_profile:
      next_level = trained_transition_from_limited_profile(method, cur_level, mtd, profile_context, THREAD);
      break;
    case CompLevel_full_profile:
      next_level = trained_transition_from_full_profile(method, cur_level, mtd, profile_context, THREAD);
      break;
  }
...
CompLevel CompilationPolicy::common(const methodHandle& method, CompLevel cur_level, jlong profile_context, JavaThread* THREAD, bool disable_feedback) {
  ...
  if (is_trivial(method) || method->is_native()) {
    ...
    if (mtd == nullptr) {
      ...
      next_level = standard_transition<Predicate>(method, cur_level, disable_feedback, profile_context);
    } else {
      next_level = trained_transition(method, cur_level, mtd, profile_context, THREAD);
      if (cur_level == next_level && !should_delay_standard_transition(method, cur_level, mtd, profile_context)) {
...
bool CompilationPolicy::should_delay_standard_transition(const methodHandle& method, CompLevel cur_level, MethodTrainingData* mtd, jlong profile_context) {
  ...
  if (highest_training_level != CompLevel_full_optimization && cur_level == CompLevel_limited_profile) {
    ...
    MethodCounters* mc = method->method_counters(profile_context);
    if (mc == nullptr) {
      return false;
    }
...
CompLevel CompilationPolicy::standard_transition(const methodHandle& method, CompLevel cur_level, bool disable_feedback, jlong profile_context) {
  ...
  default: break;
  case CompLevel_none:
    next_level = transition_from_none<Predicate>(method, cur_level, disable_feedback, profile_context);
    break;
  case CompLevel_limited_profile:
    next_level = transition_from_limited_profile<Predicate>(method, cur_level, disable_feedback, profile_context);
    break;
  case CompLevel_full_profile:
    next_level = transition_from_full_profile<Predicate>(method, cur_level, profile_context);
    break;
  }
  return next_level;
}
...
CompLevel CompilationPolicy::transition_from_none(const methodHandle& method, CompLevel cur_level, bool disable_feedback, jlong profile_context) {
  ...
  if (transition_from_full_profile<Predicate>(method, CompLevel_full_profile, profile_context) == CompLevel_full_optimization) {
...
CompLevel CompilationPolicy::transition_from_limited_profile(const methodHandle& method, CompLevel cur_level, bool disable_feedback, jlong profile_context) {
  ...
  if (next_level == CompLevel_full_profile && is_method_profiled(method, profile_context)) {
    next_level = CompLevel_full_optimization;
  }
  return next_level;
}
...
void CompilationPolicy::method_invocation_event(const methodHandle& mh, const methodHandle& imh,
  ...
  CompLevel next_level = call_event(mh, level, profile_context, THREAD);
  if (next_level != level) {
    if (is_compilation_enabled() && !CompileBroker::compilation_is_in_queue(mh)) {
      compile(mh, InvocationEntryBci, next_level, profile_context, THREAD);
    }
  }
}
...
void CompilationPolicy::method_back_branch_event(const methodHandle& mh, const methodHandle& imh,
  ...
  if (should_create_mdo(imh, level, profile_context)) {
    create_mdo(imh, profile_context, THREAD);
  }

  if (is_compilation_enabled()) {
    CompLevel next_osr_level = loop_event(imh, level, profile_context, THREAD);
    CompLevel max_osr_level = (CompLevel) imh->highest_osr_comp_level();
    // At the very least compile the OSR version
    if (!CompileBroker::compilation_is_in_queue(imh) && (next_osr_level != level)) {
      compile(imh, bci, next_osr_level, profile_context, CHECK);
    }
    ...
    if (mh() != imh()) { // If there is an enclosing method
      {
        ...
        cur_level = comp_level(mh(), profile_context);
        next_level = call_event(mh, cur_level, profile_context, THREAD);
        ...
        if (max_osr_level == CompLevel_full_optimization) {
          ...
          if (make_not_entrant) {
            if (PrintTieredEvents) {
              ...
              print_event(MAKE_NOT_ENTRANT, mh(), mh(), osr_bci, level, profile_context);
            }
